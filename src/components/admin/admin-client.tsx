...
export function AdminClient() {
  const { isAdmin, loginAdmin, logoutAdmin, adminEmail } = useAuth();
      ...
      <div className="mx-auto max-w-md space-y-4">
        <header className="text-center">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">
            حساب الأدمن: {adminEmail}
          </p>
        </header>

        <div className="rounded-xl border border-[var(--border)] bg-white p-4 shadow-[var(--shadow-sm)]">
          <label className="block text-sm">
            <span className="mb-2 block text-[var(--muted)]">رمز الأدمن</span>
            <input
              type="password"
              value={pin}
              onChange={(e) => setPin(e.target.value)}
              placeholder="أول مرة؟ سيُحفظ الرمز تلقائياً"
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
        <p className="font-medium">صلاحيات الأدمن</p>
        <ul className="mt-2 list-inside list-disc space-y-1">
          <li>عرض إحصاءات شاملة للحساب</li>
          <li>تغيير رمز الدخول للوحة الإدارة</li>
          <li>إدارة الاشتراكات والتذكيرات من كل الأقسام</li>
          <li>المستخدم العادي يرى المصروفات والاشتراكات فقط</li>
        </ul>
      </section>
    </div>
  );
